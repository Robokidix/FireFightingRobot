...
SERVOR_RIGHT=30 #Teddybot 0
SERVOR_LEFT=160 #Teddybot 140
DELAY=0.5
HIGHSPEED=100 #Teddybot 50; Goatbot 100
LOWSPEED=50
    ...
    for x in range(1):        
        ...
        if dist_front > DIST_2_F_WALL_CLOSE:
            if dist_right<DIST_2_R_WALL_TOO_FAR:
                ...
                servo(SERVOR_FRONT)
                time.sleep(0.2)
                fwd_cm_wait_avoid(18,15)
                #goatbot_fwd(20)
                goatbot_right_rot(rotation_angel)
                fwd_cm_wait_avoid(25,15)
...
def follow_wall_v2(dist_front,dist_right):
    if dist_right < DIST_2_R_WALL_FAR and dist_right > DIST_2_R_WALL_CLOSE:
        ...
        servo(SERVOR_FRONT)
        time.sleep(0.2)
        fwd_cm_wait_avoid(15,15) #how far to move each step
        #time.sleep(2)
    else:
        if dist_right > DIST_2_R_WALL_FAR:
            print("Too FAR to Right Wall: move slightly right")
...
def put_off_candle():
    ...
    if check_flame_sensor_A()==1 and check_flame_sensor_A()==1:
        ...
        for i in range (10):
	        servo(SERVOR_FRONT-20)#goatbot_right_rot(15)
	        time.sleep(1.5)   
	        servo(SERVOR_FRONT-10)#goatbot_right_rot(15)
	        time.sleep(1.5)             
	        servo(SERVOR_FRONT)#goatbot_left_rot(15)
	        time.sleep(1.5)
	        servo(SERVOR_FRONT+10)#goatbot_right_rot(15)
	        time.sleep(1.5)        
	        servo(SERVOR_FRONT+20)#goatbot_left_rot(15)        
        turn_off_fan()
        print("Task 6 : Fan is off")
        if check_flame_sensor_A()==1 and check_flame_sensor_B()==1:
