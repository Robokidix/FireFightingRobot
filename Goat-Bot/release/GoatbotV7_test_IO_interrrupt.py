...
DELAY=0.5
HIGHSPEED=80+20+20 #Teddybot 50; Goatbot 100
LOWSPEED=50
...
def put_off_candle():
    ...
    if check_flame_sensor_A()==1 and check_flame_sensor_A()==1:
        ...
        if check_flame_sensor_A()==1 and check_flame_sensor_A()==1:
            ...
            goatbot_right_rot_low(15,0)
            time.sleep(1)
            fwd_cm_wait_avoid(15,5,0)
            time.sleep(5)
            turn_off_fan()
            time.sleep(1)
            print("Task 6 : Fan is off")
...
def goatbot_right_rot_low(degree, no_flame_sensor=None):
    set_speed(LOWSPEED)
    if degree<12:
		degree=12
    #Rotate right both wheel
    right_rot_deg_wait(degree,no_flame_sensor)

def goatbot_left_rot_low(degree,no_flame_sensor=None):
    set_speed(LOWSPEED)
    if degree<12:
		degree=12
    #Rotate left both wheel
    left_rot_deg_wait(degree,no_flame_sensor)
    #time.sleep(2)
...
if __name__ == '__main__':
	RPi.GPIO.add_event_detect(20,RPi.GPIO.FALLING,callback=test_callback,bouncetime=300)
	try: 
		print("wait for pin 20")
